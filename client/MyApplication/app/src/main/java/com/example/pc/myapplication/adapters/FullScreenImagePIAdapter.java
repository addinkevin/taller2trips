...
public class FullScreenImagePIAdapter extends PagerAdapter {
    private Activity fullScreenAct;

    // constructor
    public FullScreenImagePIAdapter(Activity activity) {
        this.fullScreenAct = activity;
    }

    @Override
    public int getCount() {
        return ImagesSingletonPI.getInstance().size();
    }
    ...
    private void setImgViewPager(View viewLayout,int position){
        ...
        Bitmap myBitmap = ImagesSingletonPI.getInstance().get(position);
